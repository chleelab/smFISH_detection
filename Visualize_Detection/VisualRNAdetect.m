...
for f= 1:size(af,1)
    if isempty(af{f,9+g}) || isempty(af{f,1}) || isempty(af{f,2})|| isempty(af{f,3})|| isempty(af{f,4})
        continue
    end
%    if isempty(af{f,3})|| isempty(af{f,4})
%         continue
%     end 
%     if f == 1
%         continue
...
%         figure, imshow(blankp)
        ...
        plot(af{f,g*2}(:,1), af{f,g*2}(:,2), marker, 'markersize', 10, 'linewidth', 1);
        text(5,7, strcat(num2str(f),'-th image'), 'color', 'y', 'fontsize', 10);
%         plot(af{f,13}(:,1), af{f,13}(:,2), 'go', 'markersize', 10, 'linewidth', 1);
