function [DETrna, derna, mom] = DetectRNAExon(rPla, pixr, iminfo, f, numOfImg, g, thresForExon, gmask)
...
%     f1 = imdilate(B, se3);
%     bgTMP1 = sortrows(t1{i}(f1 == 0 & gmask2 == 1),1);
%     lt = length(bgTMP1);
%     bgRg = [1 round(lt*1/4)];
%     bgTMP = mean(bgTMP1(bgRg(1):bgRg(2)));
    bgTMP = mom;
    if isnan(temp)
        Pmofm(i) = 0;
    else
    ...
    if Pmofm(i)/bgTMP < 1.4 % cutoff for overall dots-bg ratio
        ...
        for j = 1:size(A,1)
            ...
            if ratioSB(j) < 1.15 || ratioSoverall(j,1) < 1.15 + (mupr2-1)/4  % the cutoff signal-local bg ratio of each mRNA (3x3 mRNA vs. 7x7 bg region.
                B(A(j,2), A(j,1)) = 0;
                A(j,3) = 999;
                ratioSB(j,2) = 999;
