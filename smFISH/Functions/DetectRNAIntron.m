function [blobrna, blrna, thrrna, mom] = DetectRNAIntron(rPla, pixr, iminfo, f, numOfImg, g, thresForIntron, gmask)
...
%%%----------------- peak detection method (1/3) ------------------
    ...
    if ~isempty(A)
        ...
        if Pmofm(i)/bgTMP < 1.0  % cutoff for overall dots-bg ratio
            B(A(:,2), A(:,1)) = 0;
            A(:,3) = 999;
        else
...
%                 fprintf('\n%.2f for %d', ratioSB(j), j)
                ...
                if ratioSB(j,1) < 1.0 + (mupr2-1)/2 || ratioSoverall(j,1) < 1.0 + (mupr2-1)/2  % the cutoff signal-local bg ratio of each ATS (3x3 ATS vs. 7x7 bg region).
                    B(A(j,2), A(j,1)) = 0;
                    A(j,3) = 999;
                    ratioSB(j,2) = 999;
                ...
                if ~isempty(A) && sum(sum(A)) ~= 0
                    for winSize = 2:4
                        for j=1:size(A,1)
                            ...
                            if winSize == 2
                                if cal(j,1) > 1*10^-5  || cal(j,2) < 1.05  || cal(j,3) < 1.1
                                    B(A(j,2),A(j,1)) = 0;
                                    A(j,3) = 999;
                                    caller = 0;
                                end
                            elseif winSize == 3
                                if cal(j,1) > 1*10^-10  || cal(j,2) < 1.1 || cal(j,3) < 1.1
                                    B(A(j,2),A(j,1)) = 0;
                                    A(j,3) = 999;
                                    caller = 0;
                                end
                            elseif winSize == 4
                                if cal(j,1) > 1*10^-15  || cal(j,2) < 1.15 || cal(j,3) < 1.1
                                    B(A(j,2),A(j,1)) = 0;
                                    A(j,3) = 999;
                                    caller = 0;
                                end
...
%             mpix = mean(blobrna(:,5));
            ...
            temp = blobrna(:,6) < mint * 0.1 ;
            blobrna(temp,:) = [];
        end
        % Exclude RNA spots seen only on 1 or 2 z-planes
        if isempty(blobrna) == 0
            temp = blobrna(:,4) < 2 & blobrna(:,6) < mint * 0.35;
            blobrna(temp,:) = [];
        end
