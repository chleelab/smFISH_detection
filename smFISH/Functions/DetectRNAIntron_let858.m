function [blobrna, blrna, thrrna, mom] = DetectRNAIntron_let858(rPla, pixr, iminfo, f, numOfImg, g, thresForIntron, gmask)
...
mofm = mofm * 0.65;    %%% <--- IMPORTANT for the function  "FastPeakFind", a.k.a. local peak detection!! See line 57!!!.
...
%                 fprintf('\n%.2f for %d', ratioSB(j), j)
                ...
                if ratioSB(j,1) < 1.05 + (mupr2-1)/4 || ratioSoverall(j,1) < 0.75 + (mupr2-1)/2  % the cutoff signal-local bg ratio of each ATS (3x3 ATS vs. 7x7 bg region).
                    B(A(j,2), A(j,1)) = 0;
                    A(j,3) = 999;
                    ratioSB(j,2) = 999;
